...
def clean_dataset(df, NUMBER_OF_QUESTIONS, MIN_LENGTH_ANSWER, MAX_LENGTH_QUESTION):
    mask = (
        df["answer"].apply(lambda x: isinstance(x, str) and len(x.strip()) >= MIN_LENGTH_ANSWER)
        & df["question"].apply(lambda x: isinstance(x, str) and len(x.strip()) <= MAX_LENGTH_QUESTION)
    )
